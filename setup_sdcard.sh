...
while true; do
    ...
    case $choice in 
        ...
        sudo sh -c "echo 'sudo mount -o remount,exec /download' >> sudo ~/ubuntu/root/.bashrc"
        sudo sh -c "echo 'alias sd=cd /download/' >> sudo ~/ubuntu/root/.bashrc"
        sudo sh -c "echo 'alias debug64=WINEPREFIX=~/.wine box64 wine' >>~/ubuntu/root/.bashrc"
        sudo sh -c "echo 'alias debug32=WINEPREFIX=~/.wine box86 wine' >>~/ubuntu/root/.bashrc"
        sudo sh -c "echo 'alias sd=cd /Download/' >> sudo tee -a ~/ubuntu/root/.bashrc"
        sudo sh -c "echo 'alias debug64all=WINEDEBUG=-all WINEPREFIX=~/.wine box64 wine' >>~/ubuntu/root/.bashrc"
        sudo sh -c "echo 'alias debug32all=WINEDEBUG=-all WINEPREFIX=~/.wine box86 wine' >>~/ubuntu/root/.bashrc"
        rm ~/.setup_sdcard
