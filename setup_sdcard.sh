...
while true; do
    ...
    case $choice in 
        ...
        sudo sed -i '1s/^/sudo mount -o remount,exec \/download\n/' $HOME/ubuntu/root/.bashrc
        sudo sed -i '1s/^/alias sd=cd \/download\/\n/' $HOME/ubuntu/root/.bashrc
        sudo sed -i '1s/^/alias debug64=WINEPREFIX=~\/.wine box64 wine\n/' $HOME/ubuntu/root/.bashrc
        sudo sed -i '1s/^/alias debug32=WINEPREFIX=~\/.wine box86 wine\n/' $HOME/ubuntu/root/.bashrc
        sudo sed -i '1s/^/alias debug64all=WINEDEBUG=-all WINEPREFIX=~\/.wine box64 wine\n/' $HOME/ubuntu/root/.bashrc
        sudo sed -i '1s/^/alias debug32all=WINEDEBUG=-all WINEPREFIX=~\/.wine box86 wine\n/' $HOME/ubuntu/root/.bashrc
        rm ./setup_sdcard
